...
from db import fetch_emails, fetch_phone_numbers, insert_email, insert_phone_number
...
def find_email_command(update: Update, context: CallbackContext) -> int:
    update.message.reply_text('текст для поиска e-mail адресов.')
    context.user_data['choice'] = 'email'
    return INPUT_TEXT


def find_phone_command(update: Update, context: CallbackContext) -> int:
    update.message.reply_text('текст для поиска телефонных номеров.')
